  ...
  const deleteRemainder = async (id) => {
    try {
      const { data } = await axiosInstance.delete(
        `/api/v1/DeleteRemainder/${id}`
      );
      toast.success(data.message);
      setRemainders((prev) => prev.filter((reminder) => reminder._id !== id));
    } catch (err) {
      console.log(err);
      toast.error("Error deleting remainder");
    }
  };
    ...
    <div className="w-full h-screen flex flex-col">
      ...
      <div className="flex-grow p-4 mt-10">
        <h2 className="text-5xl font-bold mb-4 text-center">
          All Medicine Remainders
        </h2>

        {loading ? (
          <p>Loading...</p>
        ) : remainders.length === 0 ? (
          <p className="text-center text-2xl">No remainders found.</p>
        ) : (
                  ...
                  <TableRow key={reminder._id}>
                    ...
                    <TableCell>
                      <Button
                        variant="contained"
                        color="error"
                        onClick={() => deleteRemainder(reminder._id)}
                      >
                        Delete
                      </Button>
                    </TableCell>
