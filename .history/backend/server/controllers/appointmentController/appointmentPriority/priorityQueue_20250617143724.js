export const priorityQueueAlgo = async (data) => {
  ...
  data.forEach((value) => {
    q.enqueue(
      createAppointment(
        value.patientId,
        value.age, // ✅ Use consistent casing (was `value.Age`)
  ...
  const queue = q.printQueue(); // ✅ This now returns the queue
  return queue;
};
...
function createPriorityQueue() {
  ...
  function printQueue() {
    ...
    return queue; // ✅ So it can be returned from priorityQueueAlgo
  }
