	...
	while (openSet.size > 0) {
		...
		for (const neighbor of graph.nodes[current].neighbors) {
			...
			if (tentativeG < gScore[neighbor.id]) {
				...
				fScore[neighbor.id] = tentativeG + haversine(graph.nodes[neighbor.id], graph.nodes[endId]);
...
export const astarAlgorithm = (req, res) => {
	...
	for (const p of pharmacies) {
		...
		if (path.length > 0) {
			const dist = path.reduce((sum, node, i, arr) =>
				i === 0 ? 0 : sum + haversine(arr[i - 1], node), 0
			);
