      ...
      <div className="flex flex-row gap-6">
        {cardData.map((item, index) => (
          <div
            ...
            className="import { useQuery } from "@tanstack/react-query";
import DoughnutChart from "./chart/Doughnut";
import axiosInstance from "./utils/AxiosInstance";
import { FaUserAlt } from "react-icons/fa";
import { FaUserDoctor } from "react-icons/fa6";
...
const Analytics = () => {
  ...
  if (isError) return <div className="flex justify-center items-center h-64 text-red-500">Error: {error.message}</div>;
  ...
  return (
    <div className="w-[80vw] mx-auto grid grid-cols-2 gap-8 mt-10">
      {/* Chart Card */}
      <ChartBlock
        title="Users Associated With Medigo"
        labels={labels}
...
export default Analytics; flex items-center h-40  bordergap-4 p-5 bg-white rounded-lg shadow-md hover:shadow-lg 
            transition-shadow duration-300 border border-gray-200 cursor-pointer"
          >
            <div>{item.icon}</div>
            <div className="text-xl font-medium">{item.text}</div>
