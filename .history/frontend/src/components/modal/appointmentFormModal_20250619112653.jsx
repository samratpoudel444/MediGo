...
async function getDoctorDetails(role) {
  try {
...
function AppointmentFormModal() {
  ...
  const { data } = useQuery({
    queryKey: ["doctorByRole", formData.doct],
    queryFn: getDoctorDetails(doctorType),
          ...
          <div className="p-5 text-center">
            ...
            <form className="space-y-4">
              <div className="flex flex-col gap-3">
                ...
                {
                    <MenuItem value={value.id}>{value.firstName + " "+ value.lastName}</MenuItem>
                })}
                  </Select>
                  {errors.doctorId && (
