...
const messages = [
  ...
  { id: 3, sender: "Emily", text: "All good. Let's catch up soon.dfsfsdfdsfdsfdsgsdgdfsdgd" },
  ...
  return (
    <div className="space-y-2">
      {messages.map((msg) => (
        <div
          key={msg.id}
          className={`p-2 rounded max-w-xs max-h-screen${
            msg.sender === "You" ? "bg-blue-200 ml-auto" : "bg-gray-200"
          }`}
        >
          <p className="text-xl">{msg.text}</p>
         {/* <span className="text">{msg.sender === "You"? "you" :""}</span> */}
        </div>
      ))}
    </div>
  );
