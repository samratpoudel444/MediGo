...
const PrescriptionUpload = () => {
  ...
  // Function to fetch RxNorm data and open modal
  const handleApi = async () => {
    const medName = mutation?.data?.extractedText?.["Medicine Name"];
    if (!medName) return toast.warning("No medicine name found.");
    ...
    try {
      ...
      if (
        ...
        data?.drugGroup?.conceptGroup[0]?.conceptProperties
      ) {
        const concepts = data.drugGroup.conceptGroup[0].conceptProperties;
        const result = concepts.map((c) => `${c.name} (${c.rxcui})`).join(", ");
        setMedicineDetails(result);
      } else {
        setMedicineDetails("No data found.");
      }
    } catch (err) {
      ...
      >
        ...
        >
          ...
          {rxLoading ? (
            ...
            <Typography sx={{ mt: 2 }}>{medicineDetails}</Typography>
          )}
        </Box>
      </Modal>
    </div>
