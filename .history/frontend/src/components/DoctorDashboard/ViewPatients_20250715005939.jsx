...
const AllPatientsTable = () => {
  ...
  const handleSendMessage = async (patient) => {
    try {
      const messageBody = {
        to: patient.email,
        message: `Hello ${patient.firstName}, this is a message from your doctor.`,
      };

      const res = await axiosInstance.post("/api/v1/sendMessage", messageBody);
      console.log("📨 Message sent:", res.data);
      alert(`Message sent to ${patient.firstName}`);
    } catch (error) {
