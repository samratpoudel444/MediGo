...
const GetAllAppointments = () => {
    ...
    <div className="">
      ...
      <div className="overflow-x-auto mt-6 mr-5 ml-5">
        <table className="table-auto w-full border border-gray-300">
          ...
          <tbody>
          {appointments.map(({ _id, patientId, appointmentDate, age, reason, appointmentType, isApproved }, idx) => (
  <tr key={_id} className="border">
    ...
    <td className="px-4 py-2">{new Date(appointmentDate).toDateString()}</td>
    <td className="px-4 py-2">{age ?? "N/A"}</td>
    <td className="px-4 py-2">{reason || "N/A"}</td>
    <td className="px-4 py-2">{appointmentTypeMap[appointmentType] || "N/A"}</td>
    <td className="px-4 py-2">{isApproved ? "Approved" : "Pending"}</td>
  </tr>
))}
