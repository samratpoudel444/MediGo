        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <h2 className="font-bold text-2xl text-gray-800">
            {userDetails?.userId.firstName +
              " " +
              userDetails?.userId.lastName || "Loading..."}
          </h2>
        </div>
        ...
        <div className="flex flex-col w-2/3 items-start justify-start gap-3">
          ...
          {userDetails ? (
            <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-gray-700 text-lg w-full">
              <p>
                <strong>First Name:</strong> {userDetails.userId.firstName}
              </p>
              <p>
                <strong>Last Name:</strong> {userDetails.userId.lastName}
              </p>
              <p>
                <strong>Date of Birth:</strong> {userDetails.userId.dob}
              </p>
              <p>
                <strong>Gender:</strong> {userDetails.userId.gender}
              </p>
              <p>
                <strong>Role:</strong> {userDetails.userId.role}
              </p>
              <p>
                <strong>Address:</strong> {userDetails.userId.address}
              </p>
              <p>
                <strong>Longitude:</strong> {userDetails.userId.longitude}
              </p>
              <p>
                <strong>Latitude:</strong> {userDetails.userId.latitude}
              </p>
              <p>
