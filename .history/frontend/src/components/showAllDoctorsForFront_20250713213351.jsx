    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <main className=" flex-grow flex justify-center flex-wrap gap-10 py-10">
          {data.map((arr, index) => (
            <div className="h-50 w-50 mx-10 my-10 rounded-2xl hover:scale-105 shadow-2xl border border-gray-300">
              <img
                className="px-2 py-2 rounded-full"
                src={arr?.image || image}
                alt=""
              />
              <span className="text-2xl text-center">{arr.userId.firstName}</span>
            </div>
          ))}
        </main>
        <Footer className="fixed bottom-0 w-full" />
