...
const handleApi = async () => {
  const medName = mutation?.data?.extractedText?.["Medicine Name"] || "Acetaminophen";
  ...
  try {
    ...
    const response = await fetch(
      `https://api.fda.gov/drug/label.json?search=openfda.brand_name:"${encodeURIComponent(medName)}"&limit=1`
    );
    ...
    const dosage = drugInfo.dosage_and_administration || ["Dosage information not available."];
    const sideEffects = drugInfo.adverse_reactions || ["Side effects information not available."];
    const warnings = drugInfo.warnings || ["Warnings information not available."];
    ...
    setMedicineDetails(
      <>
        ...
        {dosage.map((d, i) => (
          <Typography key={`dosage-${i}`} paragraph>{d}</Typography>
        ))}
        ...
        {sideEffects.map((se, i) => (
          <Typography key={`sideeffect-${i}`} paragraph>{se}</Typography>
        ))}
        ...
        {warnings.map((w, i) => (
          <Typography key={`warning-${i}`} paragraph>{w}</Typography>
        ))}
      </>
    );
  } catch (error) {
