...
const PrescriptionUpload = () => {
  ...
  // Fetch medicine details from OpenFDA API and display in modal
  const handleApi = async () => {
    ...
    try {
      ...
      const doNotUse = drugInfo.do_not_use || [];
      const askDoctor = drugInfo.ask_doctor || [];
      const askDoctorOrPharmacist = drugInfo.ask_doctor_or_pharmacist || [];
      const stopUse = drugInfo.stop_use || [];
        ...
        <>
          ...
          {doNotUse.length > 0 && (
            <>
              ...
              {doNotUse.map((d, i) => (
                <Typography key={`donotuse-${i}`} paragraph>
                  {d}
                </Typography>
              ))}
            </>
          ...
          {askDoctor.length > 0 && (
            <>
              ...
              {askDoctor.map((a, i) => (
                <Typography key={`askdoctor-${i}`} paragraph>
                  {a}
                </Typography>
              ))}
            </>
          ...
          {askDoctorOrPharmacist.length > 0 && (
            <>
              <Typography
                variant="subtitle1"
                fontWeight="bold"
              ...
              {askDoctorOrPharmacist.map((a, i) => (
                <Typography key={`askdocpharm-${i}`} paragraph>
                  {a}
                </Typography>
              ))}
            </>
          ...
          {stopUse.length > 0 && (
            <>
              ...
              {stopUse.map((s, i) => (
                <Typography key={`stopuse-${i}`} paragraph>
                  {s}
                </Typography>
              ))}
            </>
